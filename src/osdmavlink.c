...
mavlink_system_t mavlink_system = {12,1}; //Ardupilot:7,1  Pixhawk:100,50
...
{
    const u8 MAVStreams[MAX_STREAMS] = {
   ...
   // uint16_t MAVRates[MAX_STREAMS] = {0x01, 0x02, 0x05, 0x02, 0x05, 0x02};
    uint16_t MAVRates[MAX_STREAMS] = {0x64, 0x64, 0x64, 0x64, 0x64, 0x64};
   //  uint16_t MAVRates[MAX_STREAMS] = {0xc8, 0xc8, 0xc8, 0xc8, 0xc8, 0xc8};
...
//putchar1('2');
                        ...
                        //tprintf("BAT%d \r\n",osd_vbat_A);
                       // osd_vbat_A=120.0232344f;
//                         temp_a=(int32_t)osd_vbat_A;
//                        Uart_Printf("BAT %d.%d \r\n",(int32_t)osd_vbat_A,(int32_t)((osd_vbat_A-temp_a)*100));
                    }
                    break;
...
//putchar1('3');
                        ...
                        temp_a=(int32_t)osd_lat;
                       Uart_Printf("p %d.%d ",(int32_t)osd_lat,abs((int32_t)((osd_lat-temp_a)*1000000)));
                       temp_a=(int32_t)osd_lon;
                      Uart_Printf("p %d.%d \r\n",(int32_t)osd_lon,abs((int32_t)((osd_lon-temp_a)*1000000)));
                        //tprintf("GPS%d %d %d\r\n",osd_satellites_visible,(uint32_t)osd_lat,(uint32_t)osd_lon);
                    }
                    break;
...
//                        temp_a=(int32_t)osd_pitch;
//                       Uart_Printf("p %d.%d ",(int32_t)osd_pitch,abs((int32_t)((osd_pitch-temp_a)*100)));
//                       temp_a=(int32_t)osd_roll;
//                      Uart_Printf("p %d.%d \r\n",(int32_t)osd_roll,abs((int32_t)((osd_roll-temp_a)*100)));
