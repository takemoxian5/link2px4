...
#define      OV7725_OE_GPIO_PIN                       GPIO_Pin_4

// 写指针复位信号 /WRST PB12 输入
#define      OV7725_WRST_GPIO_CLK                     RCC_AHB1Periph_GPIOB
#define      OV7725_WRST_GPIO_PORT                    GPIOB
#define      OV7725_WRST_GPIO_PIN                     GPIO_Pin_12

// 读指针复位信号 /RRST PB13 输入
#define      OV7725_RRST_GPIO_CLK                     RCC_AHB1Periph_GPIOB
#define      OV7725_RRST_GPIO_PORT                    GPIOB
#define      OV7725_RRST_GPIO_PIN                     GPIO_Pin_13

// 数据输出同步时钟, RCLK PA6 输入
#define      OV7725_RCLK_GPIO_CLK                     RCC_AHB1Periph_GPIOA
#define      OV7725_RCLK_GPIO_PORT                    GPIOA
#define      OV7725_RCLK_GPIO_PIN                     GPIO_Pin_6

// 写使能信号 /WE 输入 PC2
#define      OV7725_WE_GPIO_CLK                       RCC_AHB1Periph_GPIOC
#define      OV7725_WE_GPIO_PORT                      GPIOC
#define      OV7725_WE_GPIO_PIN                       GPIO_Pin_2


// 8位数据口, D0~D7 输出 PF0~5 14~15
#define      OV7725_DATA_GPIO_CLK                     		RCC_AHB1Periph_GPIOF
...
#define FIFO_OE_H()     GPIO_SetBits(OV7725_OE_GPIO_PORT,OV7725_OE_GPIO_PIN)	  
#define FIFO_OE_L()     GPIO_ResetBits(OV7725_OE_GPIO_PORT,OV7725_OE_GPIO_PIN)	  /*拉低使FIFO输出使能*/

#define FIFO_WRST_H()   GPIO_SetBits(OV7725_WRST_GPIO_PORT,OV7725_WRST_GPIO_PIN)  /*拉高允许FIFO写(数据from摄像头)指针运动 */
#define FIFO_WRST_L()   GPIO_ResetBits(OV7725_WRST_GPIO_PORT,OV7725_WRST_GPIO_PIN)	  /*拉低使FIFO写(数据from摄像头)指针复位*/

#define FIFO_RRST_H()   GPIO_SetBits(OV7725_RRST_GPIO_PORT,OV7725_RRST_GPIO_PIN)	  /*拉高允许FIFO读(数据从FIFO输出)指针运动 */
#define FIFO_RRST_L()   GPIO_ResetBits(OV7725_RRST_GPIO_PORT,OV7725_RRST_GPIO_PIN)	  /*拉低使FIFO读(数据从FIFO输出)指针复位 */

#define FIFO_RCLK_H()   GPIO_SetBits(OV7725_RCLK_GPIO_PORT,OV7725_RCLK_GPIO_PIN)	  
#define FIFO_RCLK_L()   GPIO_ResetBits(OV7725_RCLK_GPIO_PORT,OV7725_RCLK_GPIO_PIN)	  /*FIFO输出数据时钟*/

#define FIFO_WE_H()     GPIO_SetBits(OV7725_WE_GPIO_PORT,OV7725_WE_GPIO_PIN)	  /*拉高使FIFO写允许*/
#define FIFO_WE_L()     GPIO_ResetBits(OV7725_WE_GPIO_PORT,OV7725_WE_GPIO_PIN)


#define READ_FIFO_PIXEL(RGB565)   	do{\
	                                  RGB565=0;\
	                                  FIFO_RCLK_L();\
																		RGB565=(GPIOF->IDR&0xC000)|((GPIOF->IDR&0x003F)<<8);\
	                                  FIFO_RCLK_H();\
                                    FIFO_RCLK_L();\
	                                  RGB565|=((GPIOF->IDR&0xC000)>>8)|(GPIOF->IDR&0x003F);\
	                                  FIFO_RCLK_H();\
                                    }while(0)
...
void OV7725_GPIO_Config(void);//OV7725GPIO口设置
ErrorStatus OV7725_Init(void);
void ImagDisp(uint16_t width,uint16_t height);
