...
OV7725_MODE_PARAM cam_mode =
{
	...
	.cam_width = 120, 
	.cam_height = 120, //在VGA模式下，此值才可以大于240
...
static void FIFO_GPIO_Config(void)
{
    GPIO_InitTypeDef GPIO_InitStructure;
		GPIO_InitTypeDef GPIO_InitStructure_data;
	
	
		/*开启时钟*/
	  RCC_AHB1PeriphClockCmd (OV7725_OE_GPIO_CLK|OV7725_WRST_GPIO_CLK|
    ...
    /*(FIFO_DATA--FIFO输出数据)*/
		GPIO_InitStructure_data.GPIO_Mode = GPIO_Mode_IN;//输入
		GPIO_InitStructure_data.GPIO_PuPd = GPIO_PuPd_NOPULL;
    GPIO_InitStructure_data.GPIO_Speed = GPIO_Speed_100MHz;
		
		GPIO_InitStructure_data.GPIO_Pin = 	OV7725_DATA_0_GPIO_PIN | OV7725_DATA_1_GPIO_PIN |
		...
		GPIO_Init(GPIOF, &GPIO_InitStructure_data);
...
static void VSYNC_GPIO_Config(void)
{
	  ...
	  EXTI_InitTypeDef EXTI_InitStructure;
		...
		/*初始化引脚*/
		RCC_AHB1PeriphClockCmd(OV7725_VSYNC_GPIO_CLK,ENABLE);
    ...
    SYSCFG_EXTILineConfig(OV7725_VSYNC_EXTI_SOURCE_PORT, OV7725_VSYNC_EXTI_SOURCE_PIN);
    EXTI_InitStructure.EXTI_Line = OV7725_VSYNC_EXTI_LINE;
    EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
		EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling ; //下降沿触发
    EXTI_InitStructure.EXTI_LineCmd = ENABLE;//中断线使能
		EXTI_Init(&EXTI_InitStructure);//配置
    EXTI_GenerateSWInterrupt(OV7725_VSYNC_EXTI_SOURCE_PORT);//软件中断，不甚懂
...
void OV7725_VSYNC_EXTI_INT_FUNCTION ( void )
{
    ...
    {
				static uint8_t Ov7725_vsync=0 ;	 //帧同步信号标志
        if( Ov7725_vsync == 0 )
        {
        ...
        else if( Ov7725_vsync == 1 )
        {
            ...
            FIFO_PREPARE;  			/*FIFO准备*/					
						ImagDisp(cam_mode.cam_width, cam_mode.cam_height);			/*采集并显示*/
						Ov7725_vsync = 0;
        }        
...
ErrorStatus OV7725_Init(void)
{
	...
	{
		printf("\r\nOV7725 write data error\r\n");		
		return ERROR ;
	}	

	if( 0 == SCCB_ReadByte( &Sensor_IDCode, 1, 0x0b ) )	 /* 读取sensor ID号*/
	{
		printf("\r\nOV7725 read id failed\r\n");	
		return ERROR;
	}
	printf("\r\nOV7725 Sensor ID is 0x%x\r\n",Sensor_IDCode);
...
void OV7725_Window_Set(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height,uint8_t QVGA_VGA)
{
	...
	SCCB_ReadByte(&reg_raw,1,REG_HSTART);//读地址为REG_HSTART的一个字节，存放在reg_raw
...
/**
  ...
  * @retval 无
  */
extern unsigned int black_num[9];
...
void ImagDisp(uint16_t width,uint16_t height)
{
	...
	for(i = 0; i < height; i++)
	{
		for(j = 0; j < width; j++)
		{
			...
			LCD_Fast_DrawPoint(j,i,Camera_Data);
			//OV7725选择黑白模式，即为灰度图，RGB三个分量相等
			color_gray=Camera_Data&0x001F;
//			if(color_gray>THRESHOLD)
//				COLOR_GRAY=WHITE;
//			else
//			{
//				COLOR_GRAY=BLACK;
...
//				{
//					if(j<80)
//						black_num[0]=black_num[0]+1;
//					else if(j<160)
//						black_num[1]=black_num[1]+1;
//					else
//						black_num[2]=black_num[2]+1;
//				}
//				else if(i<160)
//				{
//					if(j<80)
//						black_num[3]=black_num[3]+1;
//					else if(j<160)
//						black_num[4]=black_num[4]+1;
//					else
//						black_num[5]=black_num[5]+1;
//				}
//				else
//				{
//					if(j<80)
//						black_num[6]=black_num[6]+1;
//					else if(j<160)
//						black_num[7]=black_num[7]+1;
//					else
//						black_num[8]=black_num[8]+1;
//				}	
//				
//			}
//			LCD_Fast_DrawPoint(j,i,COLOR_GRAY);
		}
	}
}
